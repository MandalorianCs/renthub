...
async def on_item_publish(query: CallbackQuery, state: FSMContext) -> None:
    ...
    # Про описание говорим всегда, а не только когда его нет: шага для
    # него в этом диалоге не существует вовсе, и человек не знает, что
    # оно бывает. Молчать здесь — значит оставить каждое объявление из
    # чата без ответа на «а что я получу», причём в единственный момент,
    # когда владелец точно смотрит на экран.
    text += (
        "\n\nОписание я не спрашивал — оно не обязательно, но помогает: "
        "человек чаще пишет тому, у кого понятно, что в комплекте и в каком "
        "вещь состоянии.\n\nДобавить: /вещи → «Добавить описание»."
    )
