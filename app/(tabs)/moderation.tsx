...
function DisputeCard({
  ...
  return (
    <Card style={{ borderColor: colors.danger }}>
      ...
      {/* Пустая пачка «до» — не пустяк, а причина насторожиться. Фото «до»
          лежат в папке владельца, и удалить их может он сам: если вещь уже
          была повреждена до аренды, снимок со сколом мешает заявить порчу.
          Отличить «удалил» от «не было» база не может, поэтому решение
          остаётся за модератором — но молчать об этом нельзя. */}
      {(dispute.booking?.item?.condition_photos ?? []).length === 0 ? (
        <View style={s.warn}>
          <Ionicons name="alert-circle-outline" size={17} color={colors.warn} />
          <Text style={s.warnText}>
            Фото «до» нет. Сверять «стало» не с чем: объявление опубликовали без
            снимков или владелец удалил их позже. Спросите его, прежде чем удерживать
            деньги.
          </Text>
        </View>
      ) : null}
