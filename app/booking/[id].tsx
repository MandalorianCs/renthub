...
export default function BookingScreen() {
  ...
  //
  // Оба разведены по ролям, и первый — с 04.09.2026. До этого владелец на
  // идущей аренде читал «Вернуть до» — указание, адресованное не ему:
  // возвращает арендатор, а владелец ждёт. Второй срок был разведён с
  // самого начала («Заявить о порче можно до» против «Депозит вернётся
  // до»), и рядом это выглядело как забытая половина.
  //
  // В чате обе развилки были: deadline_line в bot.py ветвится по mine, и
  // комментарий там обещает, что «одна и та же сделка не должна
  // объясняться в чате иначе, чем на экране». Обещание держал только чат.
  const deadline =
    booking.status === 'active' && booking.grace_period_ends_at
      ? {
          at: booking.grace_period_ends_at,
          color: colors.warn,
          title: isOwner ? 'Ждём возврата до' : 'Вернуть до',
          body: isOwner
            ? 'Если вещь не вернут к этому времени, система сама откроет спор ' +
              'о невозврате и удержит депозит — от вас ничего не требуется. ' +
              'Время местное.'
