    ...
    <ScrollView contentContainerStyle={s.container}>
      ...
      {isDemo ? (
        ...
        <Card>
          ...
          {/* Предупреждение — только вошедшему. У анонима isVerified тоже
              false, потому что профиля нет вовсе, и до 05.09.2026 он читал
              под ценой «Подтвердите телефон» — про шаг, которого у него ещё
              не было, и про аккаунт, которого не существует.

              Это ровно тот случай, о котором DESIGN.md говорит «разные
              состояния обязаны выглядеть по-разному»: «не вошёл» и «вошёл,
              но номер не подтверждён» решаются по-разному, а показывались
              одинаково. Первому отвечает кнопка «Войти и забронировать» в
              закреплённой панели — и она единственная, кто должен ему
              отвечать. Второму нужен этот бейдж: кнопка у него называется
              «Забронировать» и выключена, а причина без него не названа.

              Экран карточки — единственный, куда аноним вообще доходит:
              форма публикации уводит на вход, профиль тоже. Поэтому здесь
              же и единственное место, где эта разница видна человеку. */}
          {session && !isVerified ? (
