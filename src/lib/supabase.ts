...
export function humanizeError(error: unknown): string {
  ...
  const constraints: Array<[RegExp, string]> = [
    ...
    // Найдено 05.09.2026 сверкой pg_constraint с этим списком: в базе
    // семнадцать check-ограничений, переведено было шесть. Остальные
    // доходили до человека строкой `violates check constraint "..."`.
    //
    // bookings_check1 — попытка забронировать собственную вещь. Каталог
    // показывает и свои объявления, кнопка на них живая, и любопытство
