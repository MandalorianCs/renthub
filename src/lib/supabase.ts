...
export function humanizeError(error: unknown): string {
  ...
  const constraints: Array<[RegExp, string]> = [
    ...
    [/items_description_check/, 'Описание: от 2 до 600 символов, или оставьте пустым'],
    // Полное имя, а не `/daily_price_check/`: сверка ищет ограничение по
    // имени целиком, и короткая регулярка выглядела как отсутствие
    // перевода. Ловит она то же самое — но теперь это видно проверке.
    ...
    [/support_messages_text_check/, 'Слишком длинно: помещается 2000 знаков'],
    // Уникальность номера и Telegram. Человек упирается в них при привязке:
    // номер уже за кем-то числится, или этот Telegram привязан к другому
    // аккаунту. Второе бывает у тех, кто заводил аккаунт дважды.
    [/users_phone_key/, 'Этот номер уже привязан к другому аккаунту'],
    [/users_telegram_id_key/, 'Этот Telegram уже привязан к другому аккаунту'],
