...
const STEPS: { status: BookingStatus; label: string }[] = [
  ...
  // «Принята», а не «Подтверждена», и причина в ширине.
  //
  // Колонка этапа — пятая часть карточки: 71 точка на экране 420 и 65 на
  // 390. «Подтверждена» набирает 68 даже с поджатым трекингом, и на
  // распространённых телефонах подпись ломалась посреди слова:
  // «Подтвержден» / «а». Слово короче решает это на любой ширине, а не
  // до следующего телефона.
  //
  // Смысл сохранён: владелец принял заявку. Плашка статуса рядом
  // по-прежнему говорит «Подтверждено» — там места хватает.
  { status: 'confirmed', label: 'Принята' },
