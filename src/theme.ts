...
export const colors = {
  ...
  accentSoft: '#F6E7E0',
  // Акцент, которым набирают ТЕКСТ на светлом.
  //
  // Сам accent для этого не годится: на своей подложке он даёт 3,46 при
  // норме 4,5 — нашла проверка npm run check:contrast. Надписи вроде «за
  // вами ход» набраны двенадцатью пикселями, то есть ровно тем размером,
  // для которого норма и написана.
  //
  // Тон остаётся терракотовым, меняется светлота; тот же оттенок стоит на
  // сайте под именем --accent-ink, и расходиться им нельзя.
  accentInk: '#A4492C',
 ...
 */
/**
 * Наименьшая зона нажатия — 44 точки.
 *
 * ── Откуда число ─────────────────────────────────────────────
 *
 * WCAG 2.5.5 (Target Size, уровень AAA) требует 44×44. Тот же порядок
 * называют и чужие системы: разбор дизайна Airbnb, скачанный
 * `npx getdesign@latest add airbnb`, ставит главным кнопкам 48×48 и
 * прямо оговаривает, что сердечко в 32×32 — «borderline for AAA».
 *
 * ── Почему это не формальность именно у нас ──────────────────
 *
 * Инструмент берут и возвращают на улице: зимой в перчатках, летом
 * грязными руками, второй рукой придерживая перфоратор. Промах по
 * кнопке в 32 точки здесь не мелкое неудобство, а причина закрыть
 * вкладку.
 *
 * ── Почему hitSlop не считается ──────────────────────────────
 *
 * hitSlop расширяет зону нажатия только в нативной сборке.
 * react-native-web его игнорирует — проверено 08.09.2026:
 * `elementFromPoint` в шести точках от края кнопки «Поделиться»
 * (hitSlop 8, коробка 28×31) не попадает в неё вовсе. Пилот раздаётся
 * ссылкой, то есть работает как раз в вебе, и все hitSlop в коде — это
 * комментарий, а не зона нажатия.
 *
 * Поэтому размер задаётся коробкой: minHeight и minWidth. hitSlop
 * оставлен там, где стоял, — он не мешает и работает в APK.
 */
export const TAP = 44;
