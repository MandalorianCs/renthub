...
-- Анониму дверь закрыта грантом — с 06.09.2026, миграцией
-- 20260906020000_anon_execute_narrow. До неё право позвать функцию у него
-- было, и проверка внутри отвечала «нужно войти» по-русски; теперь отказ
-- приходит раньше, на входе.
--
-- Русский текст при этом никуда не делся и по-прежнему нужен: до него
-- доходит тот, у кого право есть, — вошедший с истёкшей сессией, бот с
-- пустым actor. Он проверяется ниже, отдельным утверждением, и это не
-- дубль: первое про дверь, второе про то, что за дверью.
do $$
declare v_err text;
begin
  v_err := t.anon_fails('select support_submit(''Анонимное обращение'')');
  perform t.assert(v_err like '%permission denied%',
    'анониму функция поддержки недоступна: отказ на входе');
end $$;
...
-- Анониму — тем более, и с 06.09.2026 отказ приходит на входе: право
-- звать функцию у него отобрано миграцией
-- 20260906020000_anon_execute_narrow. До неё он доходил до тела и получал
-- «нужно войти» — проверка внутри осталась и работает для того, у кого
-- право есть, но снаружи её теперь никто не пробует.
...
-- ── Лишний ноль в цене ────────────────────────────────────────
--
-- Правило «больше миллиона — проверьте, нет ли лишнего нуля» появилось
-- 02.09 в item_set_price() и там же осталось. Причина, записанная рядом с
-- ним, верна для любого способа поставить цену: объявление с такой ценой
-- никто не забронирует, а владелец узнаёт об этом не отказом, а неделей
-- тишины.
--
-- Измерено 03.09 на этом же стенде: insert с ценой 30 000 000 проходил,
-- прямой update на 99 000 000 проходил, и только item_set_price отказывала.
-- То есть создать можно было то, что нельзя установить — и починить свою
-- же опечатку правкой цены тоже было нельзя.
--
-- Проверяются все три пути. Порознь, потому что сторожат их разные вещи:
-- два первых — assert_item_price(), третий — ограничение таблицы, и
-- перепутать их значит проверить один рубеж дважды, а другой ни разу.

select t.expect_fail(t.id('owner'), format(
  'select item_set_price(%L, 30000000)', t.id('item')),
  'лишнего нуля');

select t.expect_fail(t.id('owner'),
  'select create_item(''drills'', ''Шуруповёрт с лишним нулём'', 30000000, 1000, array[''photo.jpg''])',
  'лишнего нуля');

-- Третий путь — форма правки объявления: она пишет в items напрямую,
-- политикой items_update_own. Функция такой записи не видит, и без
-- ограничения таблицы дыра осталась бы ровно там, где владелец правит
-- опечатку.
--
-- Ждать здесь исключения можно: ограничение таблицы отклоняет запрос, в
-- отличие от RLS, которая молча фильтрует строки.
select t.expect_fail(t.id('owner'), format(
  'update items set daily_price = 99000000 where id = %L', t.id('item')),
  'items_daily_price_max');

-- И то, что запрет не переусердствовал. Без этой половины «починка»
-- безусловным запретом прошла бы стенд зелёной: цену обязано быть можно
-- менять на обычную.
do $$
declare v_price integer;
begin
  ...
  perform t.as(t.id('owner'), format('select item_set_price(%L, 999999)', t.id('item')));
  perform t.assert(
    (select daily_price = 999999 from items where id = t.id('item')),
    'цена под самой границей ставится — запрет не съел обычную правку');

  perform t.as(t.id('owner'), format('select item_set_price(%L, %s)', t.id('item'), v_price));
  perform t.assert(
    (select daily_price = v_price from items where id = t.id('item')),
    'цена вернулась на место');
end $$;

-- Нижняя граница осталась там же, где была: ноль и отрицательное — та же
-- опечатка, только с другой стороны. Ограничение таблицы отказало бы и
-- само, но по-английски и именем constraint.
select t.expect_fail(t.id('owner'), format(
  'select item_set_price(%L, 0)', t.id('item')),
  'больше нуля');
