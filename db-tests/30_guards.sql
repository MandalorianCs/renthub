...
begin
  ...
  --
  -- Найдено сквозной проверкой продукта 07.09.2026: на экране отменённой
  -- заявки статус «Отменено» стоял рядом с плашкой «Депозит заблокирован»,
  -- а строкой ниже — «деньги и депозит не удерживаются». Интерфейс не врал:
  -- booking_cancel менял только status.
  --
  -- Проверка стоит здесь, а не рядом с функцией, потому что правило
  -- держится триггером и обязано работать на всех путях отмены — из
  -- приложения, из бота и из планировщика.
  perform t.assert(
    (select deposit_status = 'released' from bookings where id = t.id('booking2')),
    'отмена сняла блокировку депозита');
end $$;
...
--
-- До 06.09.2026 это проверялось прямым вызовом assert_moderator() от
-- имени анонима — право на неё у него было. Миграция
-- 20260906020000_anon_execute_narrow забрала его: функцию зовут только
-- другие функции, и снаружи она не нужна никому.
--
-- Поэтому теперь два разных утверждения вместо одного. Первое — что
-- дверь закрыта грантом; второе, ниже, — что за дверью по-прежнему
-- стоит проверка, а не пустота.
do $$
declare v_err text;
begin
  begin
  ...
  perform t.assert(v_err like '%permission denied%',
    'assert_moderator анониму недоступна: отказ приходит на входе');
end $$;
...
begin
  ...
  -- До 06.09.2026 здесь стояло `v_seen >= 5` со словами «ноль функций
  -- означал бы, что образец перестал их находить». Тогда это было верно:
  -- анониму было доступно двадцать семь функций, и проверка следила,
  -- чтобы каждая отказывала.
  --
  -- Миграция 20260906020000 отобрала право у всех, кроме двух читающих —
  -- календаря занятости и счётчика сделок. Меняющих не осталось ни одной,
  -- и ноль здесь теперь означает не сломанный образец, а закрытую дверь:
  -- второй рубеж вместо единственного.
  --
  -- Образец при этом всё равно должен уметь находить функции — иначе он
  -- зеленел бы ни на чём. Поэтому проверяется не количество действий, а
  -- то, что запрос вообще видит функции роли anon: две читающие обязаны
  -- найтись.
  perform t.assert(
    (select count(*) from pg_proc p
      where p.pronamespace = 'public'::regnamespace
        and has_function_privilege('anon', p.oid, 'EXECUTE')) >= 2,
    'образец жив: функции, доступные анониму, находятся');

  perform t.assert(v_seen = 0,
    format('меняющих функций у анонима не осталось; найдено %s', v_seen));
...
-- ── Что анониму осталось — поимённо ───────────────────────────
--
-- Список, а не правило, и это осознанно: правило «пусть будет пусто» уже
-- стоит выше, а здесь важно, чтобы РАСШИРЕНИЕ доступа было заметным.
-- Новая функция, открытая анониму по недосмотру, уронит стенд с именем.
--
-- Обе оставленные нужны до входа: без календаря арендатор выбирает даты
-- вслепую, без счётчика сделок не поймёт, кому отдаёт вещь за 90 000 ₸.
do $$
declare
  v_open text[];
begin
  ...
  perform t.assert(
    v_open = array['item_busy_dates', 'user_deals_count'],
    'анониму открыты ровно две читающие функции; сейчас: '
