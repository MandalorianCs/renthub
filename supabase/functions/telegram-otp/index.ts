 ...
 *
 * Второе не редкость: на 04.09 из восьми участников семеро ещё не привязали
 * Telegram, то есть почти каждый, кто откроет вкладку «По SMS».
 *
 * Пока это так, тексты продублированы в `humanizeError`
 * (`src/lib/supabase.ts`) — человек видит русское объяснение, но приходит
 * оно от клиента, а не отсюда. Две копии одного текста — плохо, и это
 * временно.
 *
 * **Гипотеза, которую надо проверить при следующем развёртывании**
 * (проверить в этой сессии было нечем: развёртывание функции требует
 * `npx supabase login`): контракт Send SMS Hook, судя по поведению, ждёт
 * HTTP 200 и разбирает ошибку из тела, а настоящий 4xx считает поломкой
 * хука. Тогда достаточно оставить `status: 200`, не трогая `http_code`
 * внутри. Проверять так: развернуть, запросить код на номер не участника
 * и посмотреть, придёт ли текст целиком.
 */
