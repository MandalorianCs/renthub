...
const TOOLS = [
  { title: 'Перфоратор Bosch GBH 2-26', category: 'rotary_hammers', price: 3500, deposit: 20000,
    description: 'С кейсом и тремя бурами. Есть режим долбления. Проверен, работает без нареканий.', area: 'возле вокзала' },
  { title: 'УШМ Makita 125 мм', category: 'grinders', price: 2000, deposit: 12000,
    description: 'Болгарка с регулировкой оборотов. В комплекте два отрезных диска и защитный кожух.', area: 'центр, у рынка' },
  { title: 'Бетономешалка 140 л', category: 'concrete', price: 6000, deposit: 30000,
    description: 'Для стяжки и кладки. Самовывоз, помещается в багажник универсала.', area: 'частный сектор, южная часть' },
  { title: 'Строительные леса, 4 секции', category: 'scaffolding', price: 5000, deposit: 30000,
    description: 'Высота до 6 метров. Настилы и колёса в комплекте.', area: 'выезд на трассу, склад' },
  { title: 'Шуруповёрт DeWalt 18V', category: 'drills', price: 1500, deposit: 10000,
    description: 'Два аккумулятора и зарядка. Набор бит на 30 предметов.', area: 'центр' },
  { title: 'Дисковая пила Metabo', category: 'saws', price: 2500, deposit: 15000,
    description: 'Диск 190 мм, направляющая шина. Для распила доски и фанеры.', area: 'возле автовокзала' },
  { title: 'Лазерный уровень 360°', category: 'measuring', price: 2000, deposit: 15000,
    description: 'Самовыравнивающийся, со штативом. Хватает на комнату целиком.', area: 'центр' },
  { title: 'Виброплита 90 кг', category: 'concrete', price: 8000, deposit: 40000,
    description: 'Для трамбовки грунта и укладки брусчатки. Бензиновая, заправлена.', area: 'частный сектор, южная часть' },
];
...
for (let i = 0; i < TOOLS.length; i++) {
  ...
  const { error } = await admin.from('items').insert({
    ...
    // Ориентиры нейтральные — «возле вокзала», «центр»: выдуманный
    // микрорайон в демо на питче читался бы как настоящий адрес.
    pickup_area: tool.area ?? null,
    city,
  });
