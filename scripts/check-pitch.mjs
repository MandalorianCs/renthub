...
for (const [label, file, command] of [
  ...
  if (builtFrom !== deckNow) {
    ...
    // Вторая причина того же отказа, и в CI она вероятнее первой:
    // отпечаток лежит ОТДЕЛЬНЫМ файлом рядом, и его легко не добавить в
    // коммит. Локально проверка при этом зелёная — в рабочей копии .sha
    // свежий, — и расхождение видит только CI. 07.09.2026 так и вышло:
    // дека, PDF и PPTX уехали, а два .sha остались дома, и шаг «Числа
    // деки» падал сообщением про вчерашние числа, которых не было.
    if (!builtFrom) {
      console.log('      файла отпечатка нет вовсе: ' + shaPath);
    }
    console.log(`      в коммит идут четыре файла: PDF, PPTX и оба .sha рядом с ними`);
    failed++;
    continue;
  }
...
if (secret) {
  ...
  // ── Стадия: что говорим против того, что в базе ─────────────
  //
  // 08.09.2026 Глеб спросил коротко: «у нас только MVP же, пилота нет?» —
  // и был прав. В базе на тот момент: одна бронь, и та отменена, ноль
  // отзывов, одно живое объявление из девяти. А лендинг обещал «живые
  // сделки», и в трёх местах стояло «пилот идёт».
  //
  // Разница между «продукт развёрнут» и «пилот идёт» — это разница между
  // стадией и результатом, и судья проверяет её первым вопросом: сколько
  // сделок прошло. Поймать такое пересказом невозможно — только счётом.
  //
  // Слова выбраны узкие: «живые сделки» и «пилот идёт» утверждают
  // происходящее. «Пилот в Кокшетау» как название этапа и «на этапе
  // пилота деньги не двигаются» ничего не обещают и сюда не попадают.
  {
    console.log('');
    console.log('── Стадия против базы ──');

    const [bookings, reviews, items] = await Promise.all([
    ...
    if (bookings.error) {
      console.log(`  ??  брони не прочитать: ${bookings.error.message}`);
      failed++;
    } else {
      const all = bookings.data ?? [];
      const done = all.filter((b) => b.status === 'completed').length;

      console.log(`  ··  в базе: броней ${all.length}, из них закрытых ${done}, ` +
        `отзывов ${reviews.count ?? 0}, объявлений ${items.count ?? 0}`);

      // Образцы узкие намеренно, и один пришлось сузить сразу же:
      // «сделки идут» ловило описание схемы — «шесть шагов сделки идут по
      // кругу», — то есть срабатывало на картинке, а не на обещании.
      // Проверка, которая ошибается на своём же тексте, учит себя не
      // читать; лучше пропустить формулировку, чем звать волков.
      const claims = [
        [/живые сделки/i, 'живые сделки'],
        [/пилот идёт/i, 'пилот идёт'],
        [/сделки уже идут/i, 'сделки уже идут'],
      ];

      for (const [pattern, what] of claims) {
        const where = [];
        if (pattern.test(pitch)) where.push('дека');
        if (pattern.test(landing)) where.push('лендинг');
        if (where.length === 0) continue;

        if (done > 0) {
          console.log(`  ok  «${what}» — подтверждено: закрытых сделок ${done}`);
        } else {
          console.log(`  ??  «${what}» обещает ${where.join(' и ')}, а закрытых сделок ${done}`);
          console.log('      это первый вопрос судьи; стадию честнее назвать запуском');
          failed++;
        }
      }

      if (!claims.some(([re]) => re.test(pitch) || re.test(landing))) {
        console.log('  ok  происходящих сделок нигде не обещаем');
      }
    }
  }
}
...
if (!existsSync(countPath)) {
  ...
  // Та же цифра словами — и это не одно место, а три.
  //
  // stat-v на «Результатах» сверялся с 06.09.2026, а прозу не смотрел
  // никто: «376 автоматических проверок» на слайде команды и «376
  // проверок бизнес-правил» на визитке живут отдельными строками.
  // Цифра растёт сама с каждым новым сценарием стенда, и разойтись
  // копии обязаны — вопрос только когда.
  //
  // Образец узкий намеренно: «проверок бизнес-правил» — это про стенд
  // и ни про что другое. Слово «проверок» само по себе в деке значит
  // и проверку вещи при возврате, и проверку номера.
  const prose = [...pitch.matchAll(/(\d+)\s+(?:автоматических\s+)?проверок бизнес-правил/g)];

  for (const [, said] of prose) {
    if (Number(said) === measured.проверок) continue;
    console.log(`  ??  в тексте деки ${said} проверок, на стенде ${measured.проверок}`);
    failed++;
  }

  if (prose.length > 0 && prose.every(([, said]) => Number(said) === measured.проверок)) {
    console.log(`  ok  та же цифра словами в ${prose.length} местах деки`);
  }
}
