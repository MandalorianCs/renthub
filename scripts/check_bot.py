...
# ── Меню Telegram против обработчиков ────────────────────────
#
# MENU — то, что бот показывает в списке команд при старте: человек
# открывает его и выбирает из готового. Команда, попавшая туда без
# обработчика, отвечает «не понял» — то есть продукт сам предлагает
# нажать то, чего не умеет.
#
# Это не выдуманный риск: список меню живёт в одном конце файла, а
# обработчики в другом, и добавляются они разными правками. Проверка
# 05.09.2026 нашла, что все девять на месте, — но нашла её команда, а не
# память.
#
# Обратную сторону — обработчик без пункта меню — не проверяем: так
# сделана /отмена, и намеренно. Она нужна внутри диалога, а в списке
# команд была бы шумом для того, кто ни в каком диалоге не находится.

# Исходник читается заново: выше он разобран в дерево (tree), а здесь
# нужен текст — регулярки по нему честнее обхода дерева ради строковых
# литералов в декораторах.
#
# Путь собирается заново, а не берётся из `path`. Переменная с этим
# именем живёт на верхнем уровне и переопределяется по дороге — цикл по
# файлам документации оставляет в ней bot/README.md. Первая версия этой
# проверки искала меню там и честно сообщала, что меню не найдено:
# образец сломан, а выглядело как «в боте нет команд».
#
# Тот же класс, что и всё, что мы сегодня чинили: имя, значащее в двух
# местах разное.
bot_source = io.open(ROOT / "bot" / "bot.py", encoding="utf-8").read()

menu = set(re.findall(r'BotCommand\(command="([a-z]+)"', bot_source))

# Обработчики регистрируются двумя способами: F.text.in_({...}) для
# пары «латиница + кириллица» и Command(...) там, где у команды есть
# аргумент. Собираем оба, иначе половина команд выглядела бы забытой.
handled = set()
for group in re.findall(r"F\.text\.in_\(\{([^}]*)\}\)", bot_source):
    handled |= {c.strip().strip("\"'").lstrip("/") for c in group.split(",") if c.strip()}
for group in re.findall(r"Command\(([^)]*)\)", bot_source):
    handled |= {c.strip().strip("\"'").lstrip("/") for c in group.split(",") if c.strip()}
# CommandStart() — отдельный декоратор без имени в скобках.
if "CommandStart()" in bot_source:
    handled.add("start")

orphans = sorted(menu - handled)

if orphans:
    failed = True
    print(f"\n\u2717 В меню Telegram есть команды без обработчика: {', '.join(orphans)}")
    print("  Человек выберет их из готового списка и получит «не понял».")
elif not menu:
    failed = True
    print("\n\u2717 Меню Telegram не найдено — образец сломан, проверка зеленела бы ни на чём")
else:
