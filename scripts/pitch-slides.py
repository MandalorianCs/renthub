        ...
        for line in lines if font else []:
            ...
            try:
                page.insert_text(
                    ...
                    #
                    # Строки слоя — отдельные текстовые объекты, и при
                    # копировании соседние слипаются: «db-tests» и «и»
                    # превращались в «db-testsи». Нашла это проверка
                    # орфографии, которая читает готовый PDF, — в самом
                    # файле слипание невидимо, зато мешает и копированию,
                    # и поиску.
                    text + " ",
                    fontname=font,
